...
class AuthService with ChangeNotifier {
  ...
  Future<bool> login(String email, String password) async {
    ...
    var request = http.Request('POST', Uri.parse('${Environment.apiUrl}/login'));
    ...
    if (response.statusCode == 200) {
      final loginResponse = loginResponseFromJson(await response.stream.bytesToString());
  ...
  Future register(String nombre, String email, String password, String sintoma, String genero, String edad) async {
    ...
    final data = {'email': email, 'password': password, 'nombre': nombre, 'sintoma': sintoma, 'genero': genero, 'edad': edad};

    var headers = {'Content-Type': 'application/json'};
    var request = http.Request('POST', Uri.parse('${Environment.apiUrl}/login/new'));
    request.body = jsonEncode(data);
    request.headers.addAll(headers);
    ...
    if (response.statusCode == 200) {
      ...
      return resBody['msg'] != null ? resBody['msg'] : "Ingrese todos los datos correctamente";
    }
  }
  ...
  Future<bool> isLoggedIn() async {
    ...
    final resp = await http.get('${Environment.apiUrl}/login/renew', headers: {'x-token': token});
